...
module.exports = ({ template }) => {
  ...
  function addModuleExportsDefaults(path) {
    ...
    if (path.get('right').isIdentifier()) {
      ...
      let replacements = [];
      const identifier = path.get('right').node.name;
      // const variable = path.scope.getBinding(identifier).path;

      replacements.push(
        template(`
          if (IMPORT.constructor.name === "Object") { 
            _exports = Object.assign(_exports, IMPORT); 
          }`
        )({IMPORT: identifier })
      );
      ...
      replacements.push(
        template(`
          if (IMPORT && {}.toString.call(IMPORT) === '[object Function]') {
            _exports[IMPORT.name] = IMPORT;
      ...
      finder.getRootPath(path).insertAfter(replacements);
    }
  }
